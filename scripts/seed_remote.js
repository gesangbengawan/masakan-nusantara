...
async function seed() {
    console.log("Mulai menanam data (Seeding)...");

    const commonIngredients = ["Bahan Utama", "Bumbu Halus", "Bumbu Pelengkap"];
    const commonInstructions = ["Siapkan semua bahan.", "Tumis bumbu hingga harum.", "Masak hingga matang dan sajikan."];
    const defaultImage = "/images/hero.png";

    let totalInserted = 0;

    for (const prov of provinces) {
        console.log(`Processing ${prov.name}...`);
        for (const rec of prov.recipes) {
            ...
            if (error) {
                console.error(`Gagal insert ${rec.title}:`, error.message);
            } else {
                totalInserted++;
            }
        }
    ...
    console.log(`Selesai! Berhasil insert/upsert ${totalInserted} resep.`);
}
