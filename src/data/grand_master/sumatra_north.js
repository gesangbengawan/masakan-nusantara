module.exports = [
    // --- NANGGROE ACEH DARUSSALAM (3 Recipes) ---
    {
        slug: "mie-aceh-spesial",
        province: "Nanggroe Aceh Darussalam",
        title: "Mie Aceh Spesial (Kuah Kental)",
        description: "Mie kuning tebal khas Aceh yang disajikan dengan kuah kari kental yang gurih dan pedas, lengkap dengan kepiting atau daging sapi. Aroma rempah kapulaga, jinten, dan daun kari (temurui) sangat menonjol.",
        image: "nanggroe_aceh_darussalam_mie_aceh_spesial.jpeg", // Placeholder for user generation
        prepTime: "30 Menit",
        cookTime: "45 Menit",
        servings: 4,
        difficulty: "Sedang",
        ingredients: [
            "## Bahan Utama",
            "500 gr Mie Kuning Basah (Mie Lidi)",
            "200 gr Daging Sapi (Iris tipis) / Kepiting",
            "100 gr Tauge & Kol Iris",
            "## Bumbu Halus (Kari)",
            "10 Cabai Merah Keriting + 5 Cabai Kering",
            "8 Bawang Merah & 4 Bawang Putih",
            "3 butir Kemiri sangrai",
            "1 sdt Jinten & 1 sdt Ketumbar sangrai",
            "3 butir Kapulaga (ambil bijinya)",
            "2 cm Kunyit & 1 cm Jahe",
            "## Aromatik",
            "2 tangkai Daun Kari (Salam Koja) - Wajib",
            "1 batang Kayu Manis & 2 Bunga Lawang"
        ],
        instructions: [
            "**Tumis Bumbu**: Panaskan minyak. Tumis bawang merah iris, kayu manis, bunga lawang, dan DAUN KARI sampai harum. Masukkan bumbu halus. Tumis sampai matang berminyak dan merah pekat.",
            "**Masak Protein**: Masukkan daging sapi/kepiting. Aduk rata. Tuang sedikit air kaldu. Masak sampai daging empuk dan bumbu meresap (nyemek).",
            "**Masukkan Mie**: Masukkan kol dan mie kuning. Aduk cepat. Tambahkan Kecap Manis dan Kecap Asin. Tuang air/kaldu lagi sesuai selera kekentalan (Mie Aceh biasanya 'nyemek' atau sedikit kuah kental).",
            "**Finishing**: Masukkan tauge dan tomat iris sesaat sebelum diangkat agar tetap renyah. Koreksi rasa (harus pedas gurih berempah).",
            "**Penyajian**: Sajikan dengan Emping Melinjo, Acar Bawang Merah, dan Timun."
        ],
        tips: [
            "**Daun Kari**: Jangan skip daun ini. Tanpa daun kari (temurui), rasanya bukan Mie Aceh tapi mie goreng biasa.",
            "**Bumbu Ebi**: Jika ingin lebih gurih, tambahkan sedikit udang ebi kering yang dihaluskan ke dalam bumbu.",
            "**Mie Lidi**: Gunakan mie lidi (mie hokkian tebal) agar tidak lembek saat dimasak kuah, teksturnya lebih 'al dente'.",
            "**Cabai Kering**: Campuran cabai kering giling membuat warna kuah merah gelap cantik dan tidak pucat.",
            "**Acar Bawang**: Sajikan wajib dengan acar bawang merah utuh untuk menetralkan rasa kari yang 'berat'."
        ]
    },
    {
        slug: "ayam-tangkap-aceh",
        province: "Nanggroe Aceh Darussalam",
        title: "Ayam Tangkap (Ayam Goreng Rempah Daun)",
        description: "Ayam goreng kampung yang dimasak bersama aneka rempah dedaunan seperti daun pandan, daun kari dan cabai hijau yang digoreng garing. Rasanya gurih meresap sampai ke tulang dengan aroma wangi dedaunan.",
        image: "nanggroe_aceh_darussalam_ayam_tangkap.jpeg",
        prepTime: "20 Menit",
        cookTime: "30 Menit",
        servings: 4,
        difficulty: "Mudah",
        ingredients: [
            "1 ekor Ayam Kampung (Potong kecil 16-20 bagian)",
            "1 Liter Minyak Goreng",
            "## Dedaunan (Ikut Digoreng)",
            "5 lembar Daun Pandan (Iris kasar)",
            "10 tangkai Daun Kari (Petik daunnya)",
            "5 buah Cabai Hijau Keriting (Utuh/Patahkan)",
            "## Bumbu Ungkep/Marinasi",
            "5 siung Bawang Putih & 3 Bawang Merah",
            "2 cm Kunyit & 1 cm Jahe",
            "1 sdt Ketumbar bubuk",
            "Air Asam Jawa & Garam"
        ],
        instructions: [
            "**Marinasi Ayam**: Bumbui potongan ayam dengan bumbu halus (bawang, kunyit, jahe, ketumbar) dan air asam jawa. Remas-remas. Diamkan 15-30 menit.",
            "**Ungkep (Opsional)**: Bisa diungkep sebentar dengan sedikit air sampai air menyusut (agar ayam empuk), atau langsung digoreng jika ayamnya muda.",
            "**Goreng Bertahap**: Panaskan minyak banyak. Goreng ayam sampai setengah matang.",
            "**Masukkan Daun**: Saat ayam mulai kekuningan, MASUKKAN semua dedaunan (pandan, kari, cabai hijau). Goreng bersama ayam.",
            "**Angkat**: Angkat saat ayam matang kecokelatan dan dedaunan sudah Garing (Garing Kering). Tiriskan bersamaan.",
            "**Sajikan**: Sajikan ayam tertimbun di bawah tumpukan daun rempah goreng yang krispi."
        ],
        tips: [
            "**Ayam Kecil**: Potong ayam ukuran kecil-kecil agar cepat matang dan bumbu meresap maksimal (finger food style).",
            "**Daun Garing**: Dedaunan rempah (pandan & salam koja) yang digoreng garing rasanya seperti keripik dan sangat enak dimakan bersama nasi.",
            "**Api**: Gunakan api sedang. Jika api terlalu besar, daun gosong tapi ayam belum matang.",
            "**Minyak**: Minyak sisa menggoreng ayam tangkap sangat wangi, enak dipakai untuk menumis nasi goreng.",
            "**Ayam Kampung**: Wajib pakai ayam kampung muda, ayam negeri akan terlalu lembek dan basah."
        ]
    },
    {
        slug: "gulai-pliek-u",
        province: "Nanggroe Aceh Darussalam",
        title: "Gulai Kuah Pliek U (Sayur Khas Aceh)",
        description: "Masakan warisan leluhur Aceh yang kaya akan sayuran, dimasak dengan bumbu khas dari 'Pliek U' (ampas kelapa fermentasi/patarana) yang dijemur. Rasanya unik: gurih, sedikit asam, dan beraroma khas.",
        image: "nanggroe_aceh_darussalam_gulai_kuah_pliek_u.jpeg",
        prepTime: "40 Menit",
        cookTime: "1 Jam",
        servings: 6,
        difficulty: "Sulit (Bahan Langka)",
        ingredients: [
            "## Bumbu Utama",
            "100 gr PLIEK U (Patarana) - Rendam air panas, blender halus",
            "500 ml Santan Encer + 200 ml Santan Kental",
            "## Aneka Sayuran (Potong Kecil)",
            "Daun Melinjo & Buah Melinjo",
            "Kacang Panjang",
            "Nangka Muda (Rebus dulu)",
            "Pepaya Muda",
            "Terong Hijau",
            "Udang Kecil (untuk kaldu)",
            "## Bumbu Halus",
            "6 Bawang Merah, 3 Bawang Putih",
            "7 Cabai Rawit, 2 cm Kunyit, 1 sdt Ketumbar",
            "Asam Sunti (Belimbing wuluh kering) - Wajib"
        ],
        instructions: [
            "**olah Pliek U**: Pastikan Pliek U sudah direndam dan diblender halus. Saring jika perlu agar tidak berpasir.",
            "**Rebus Sayur**: Rebus sayuran keras (melinjo, nangka) dan udang dengan sedikit air dan bumbu halus sampai lunak.",
            "**Masak Santan**: Masukkan Pliek U halus dan santan encer. Aduk rata. Masak sampai mendidih.",
            "**Masukkan Sayur Lunak**: Masukkan sisa sayuran (kacang panjang, daun melinjo). Tuang santan kental.",
            "**Simmering**: Masak dengan api kecil sambil diaduk agar santan tidak pecah. Bumbui garam dan gula. Wangi Pliek U akan keluar.",
            "**Sajikan**: Sajikan dengan Ikan Asin goreng."
        ],
        tips: [
            "**Pliek U**: Tidak ada substitusi untuk Pliek U. Ini adalah jiwa dari masakan ini (ampas minyak kelapa yang difermentasi).",
            "**Asam Sunti**: Memberikan rasa asam segar yang berbeda dari asam jawa. Jangan ganti cuka.",
            "**Sayuran**: Jenis sayuran bisa disesuaikan, tapi daun melinjo dan nangka muda adalah yang paling klasik.",
            "**Patarana**: Nama lain Pliek U adalah Patarana. Pastikan tidak apek sebelum dimasak.",
            "**Ikan Asin**: Jodoh wajib Kuah Pliek U adalah Ikan Asin goreng kering."
        ]
    },

    // --- SUMATERA UTARA (3 Recipes) ---
    {
        slug: "arsik-ikan-mas-toba",
        province: "Sumatera Utara",
        title: "Arsik Ikan Mas (Bumbu Kuning Batak)",
        description: "Ikan mas bumbu kuning khas Batak Toba yang dimasak hingga kering (asat) dengan cita rasa getir dan segar dari Andaliman (Merica Batak) dan Asam Gelugur.",
        image: "sumatera_utara_arsik_ikan_mas.jpeg",
        prepTime: "40 Menit",
        cookTime: "1 Jam",
        servings: 4,
        difficulty: "Sedang",
        ingredients: [
            "1 kg Ikan Mas (Biarkan sisiknya, buang isi perut)",
            "## Bumbu Khas Batak",
            "1 sdm ANDALIMAN (Tumbuk halus) - Wajib",
            "10 buah Asam Cekala (Patikala/Buah Kecombrang) - Memarkan",
            "2 buah Bunga Kecombrang (Rias) - Belah",
            "10 batang Lokio (Bawang Batak)",
            "5 keping Asam Gelugur",
            "Kacang Panjang (untuk alas)",
            "## Bumbu Halus",
            "10 Cabai Merah, 8 Bawang Merah, 5 Bawang Putih",
            "5 cm Kunyit, 4 cm Jahe, 4 cm Lengkuas",
            "5 butir Kemiri"
        ],
        instructions: [
            "**Alasi Wajan**: Susun kacang panjang, serai, dan asam cekala di dasar wajan (agar ikan tidak gosong/lengket).",
            "**Isi Perut Ikan**: Lumuri ikan dengan bumbu halus. Masukkan sebagian bumbu, kacang panjang, dan lokio ke dalam perut ikan.",
            "**Susun Ikan**: Letakkan ikan di atas tumpukan sayuran di wajan.",
            "**Siram Bumbu**: Tuang sisa bumbu halus dan air sampai ikan terendam. Taburkan ANDALIMAN tumbuk, asam gelugur, dan bunga kecombrang.",
            "**Masak Kering (Arsik)**: Masak dengan api sedang sampai air mendidih. Lalu kecilkan api. Tutup wajan. Masak terus sampai air MENYUSUT HABIS (kering) dan bumbu menempel tebal di ikan. Jangan dibalik agar ikan tidak hancur.",
            "**Penyajian**: Sajikan ikan utuh dengan sisa bumbu keringnya."
        ],
        tips: [
            "**Andaliman**: Memberikan sensasi 'getir' atau mati rasa di lidah yang eksotis. Kunci masakan Batak.",
            "**Sisik Ikan**: Tradisi Arsik membiarkan sisik ikan mas tetap ada, agar daging tidak hancur saat dimasak lama dan sisiknya bisa dimakan (krispi/kenyal).",
            "**Asam Cekala**: Buah kecombrang ini memberikan rasa asam segar yang khas. Geprek agar sarinya keluar.",
            "**Jangan Dibalik**: Ikan Mas mudah hancur. Cukup siram-siram kuah ke atas ikan saat memasak.",
            "**Waktu Masak**: Semakin lama dimasak (api kecil) semakin enak bumbunya meresap."
        ]
    },
    {
        slug: "soto-medan-premium",
        province: "Sumatera Utara",
        title: "Soto Medan (Kuah Santan Kuning)",
        description: "Soto ayam/daging bersantan dengan warna kuning pekat dan aroma rempah jinten yang kuat, disajikan lengkap dengan perkedel kentang dan sambal rawit hijau.",
        image: "sumatera_utara_soto_medan.jpeg",
        prepTime: "40 Menit",
        cookTime: "1 Jam",
        servings: 4,
        difficulty: "Sedang",
        ingredients: [
            "1/2 ekor Ayam Kampung / 500gr Daging Sapi",
            "1 Liter Santan (Kekentalan sedang)",
            "500 ml Kaldu (Rebusan ayam)",
            "2 batang Serai, 3 Daun Jeruk, 2 Bunga Lawang, 3 Kapulaga",
            "## Bumbu Halus",
            "8 Bawang Merah & 4 Bawang Putih",
            "4 cm Kunyit (Bakar)",
            "1 sdt Jinten (Sangrai) - Dominan",
            "1 sdt Ketumbar, 1/2 sdt Merica",
            "## Pelengkap",
            "Perkedel Kentang, Tauge Seduh, Bihun",
            "Bawang Goreng, Sambal Rawit Hijau"
        ],
        instructions: [
            "**Rebus**: Rebus ayam/daging dengan rempah daun sampai empuk. Angkat ayam, goreng sebentar, lalu suwir.",
            "**Tumis Bumbu**: Tumis bumbu halus, bunga lawang, dan kapulaga sampai harum matang. Masukkan ke dalam panci kuah kaldu.",
            "**Tuang Santan**: Masukkan santan. Masak sambil diaduk timba-balik agar santan tidak pecah. Bumbui garam gula.",
            "**Koreksi Rasa**: Kuah harus gurih santan dan wangi jinten/rempah.",
            "**Penyajian**: Tata bihun, tauge, suwiran ayam di mangkuk. Siram kuah panas. Sajikan dengan Perkedel dan Sambal."
        ],
        tips: [
            "**Jinten & Bunga Lawang**: Dua rempah ini yang membedakan aroma Soto Medan dengan soto Jawa (Lamongan/Kudus).",
            "**Santan Segar**: Gunakan santan peras segar untuk rasa manis alami. Santan instan rasanya kurang pas.",
            "**Perkedel**: Perkedel kentang adalah lauk pendamping yang wajib ada untuk Soto Medan.",
            "**Sambal Rawit Hijau**: Gunakan cabai rawit hijau yang digiling halus, bukan sambal merah.",
            "**Kunyit**: Bakar kunyit sebelum dihaluskan agar warna kuningnya cantik dan tidak langu."
        ]
    },
    {
        slug: "bika-ambon-medan",
        province: "Sumatera Utara",
        title: "Bika Ambon (Kuning Kenyal)",
        description: "Kue basah tradisional berwarna kuning dengan tekstur bersarang (berongga) yang kenyal, manis, dan beraroma harum daun jeruk dan serai. Dimasak dengan oven/cetakan pasir.",
        image: "sumatera_utara_bika_ambon.jpeg",
        prepTime: "2 Jam (Fermentasi)",
        cookTime: "1 Jam",
        servings: 10,
        difficulty: "Sulit (Teknik Sarang)",
        ingredients: [
            "## Bahan Santan (Rebus & Dinginkan)",
            "300 ml Santan Kental",
            "10 lembar Daun Jeruk (Sobek)",
            "2 batang Serai (Geprek)",
            "1 sdt Kunyit Bubuk (Pewarna alami)",
            "## Bahan Biang (Pengembang)",
            "1 sdt Ragi Instan (Fermipan)",
            "50 ml Air hangat + 1 sdt Gula",
            "## Adonan Utama",
            "150 gr Tepung Sagu/Tapioka",
            "200 gr Gula Pasir",
            "8 butir Kuning Telur (Wajib banyak kuning telur)",
            "2 butir Telur Utuh"
        ],
        instructions: [
            "**Masak Santan**: Rebus santan dengan daun jeruk, serai, kunyit, dan garam sampai mendidih. Saring dan DINGINKAN suhu ruang.",
            "**Buat Biang**: Larutkan ragi. Diamkan 10 menit sampai berbuih.",
            "**Kocok Telur**: Kocok telur dan gula pasir sampai larut (tidak perlu fluffy). Masukkan tepung sagu dan santan selang seling sambil diaduk rata (jangan overmix).",
            "**Fermentasi**: Masukkan bahan biang. Aduk rata. Tutup kain basah. Diamkan 2-3 jam sampai muncul banyak gelembung busa.",
            "**Panggang (Kunci Sarang)**: Panaskan cetakan/loyang berisi pasir di bawahnya (agar panas merata). Oles minyak. Tuang adonan. PANGGANG DENGAN PINTU OVEN SEDIKIT TERBUKA (api bawah saja) sampai permukaan berlubang-lubang dan kering. Jangan ditutup rapat agar sarang terbentuk.",
            "**Matangkan**: Setelah bersarang dan atasnya mulai set, baru tutup oven atau nyalakan api atas sebentar untuk warna kecokelatan."
        ],
        tips: [
            "**Kunci Sarang**: Panas harus dari bawah dan stabil. Pintu oven dibuka sedikit membantu gelembung udara naik dan tidak pecah (collapse).",
            "**Daun Jeruk**: Jumlah daun jeruk yang banyak kuncinya wangi Bika Ambon.",
            "**Jangan Diaduk Saat Panggang**: Jangan menggoyangkan loyang saat proses 'bersarang' terjadi.",
            "**Santan Kental**: Gunakan santan kani (kepala santan) agar kue berminyak dan legit.",
            "**Pasir**: Meletakkan loyang berisi pasir di rak paling bawah oven membantu menahan panas agar stabil (teknik tradisional)."
        ]
    },

    // --- RIAU (3 Recipes) ---
    {
        slug: "gulai-ikan-patin-riau",
        province: "Riau",
        title: "Gulai Ikan Patin (Kuah Kuning)",
        description: "Perpaduan ikan patin sungai yang lembut dengan kuah gulai kuning yang dicampur tempoyak (durian fermentasi) atau hanya asam kandis, menghasilkan rasa gurih creamy.",
        image: "riau_gulai_ikan_patin_tempoyak.jpeg",
        prepTime: "20 Menit",
        cookTime: "30 Menit",
        servings: 4,
        difficulty: "Sedang",
        ingredients: [
            "1 kg Ikan Patin (Potong, cuci jeruk nipis)",
            "700 ml Santan Sedang",
            "2 lembar Daun Kunyit (Ikat)",
            "2 keping Asam Kandis",
            "## Bumbu Halus",
            "8 Bawang Merah, 4 Bawang Putih",
            "4 cm Kunyit, 3 cm Jahe, 3 cm Lengkuas",
            "15 Cabai Merah Keriting (Warna merah cantik)",
            "## Opsi Tempoyak",
            "2 sdm Tempoyak (Jika suka aroma durian)"
        ],
        instructions: [
            "**Masak Santan**: Masak santan, bumbu halus, daun kunyit, dan asam kandis di wajan. Aduk terus sampai mendidih dan bumbu matang (hilang langu).",
            "**Tambahkan Tempoyak**: Jika pakai tempoyak, masukkan sekarang. Aduk sampai larut.",
            "**Masukkan Ikan**: Masukkan potongan ikan patin. Masak dengan api kecil. Jangan terlalu sering diaduk agar ikan tidak hancur.",
            "**Matangkan**: Masak sampai kuah berminyak dan ikan matang. Koreksi rasa garam dan gula.",
            "**Sajikan**: Sajikan dengan nasi hangat dan lalapan pucuk ubi rebus."
        ],
        tips: [
            "**Patin Sungai**: Pilih patin sungai (bukan tambak) jika ada, dagingnya lebih manis dan tidak bau tanah.",
            "**Daun Kunyit**: Aroma daun kunyit sangat penting di masakan Melayu Riau.",
            "**Cuci Ikan**: Cuci patin dengan air jeruk nipis dan garam untuk menghilangkan lendir dan bau amis tanah.",
            "**Warna Kuah**: Kuah gulai patin Riau biasanya berwarna kuning kemerahan cantik, tidak pucat.",
            "**Tempoyak**: Jika tidak suka durian, tempoyak bisa di-skip, rasanya tetap enak (Gulai Kuning)."
        ]
    },
    {
        slug: "asam-pedas-baung-riau",
        province: "Riau",
        title: "Asam Pedas Ikan Baung",
        description: "Ikan baung sungai segar yang dimasak dengan kuah merah menyala yang sangat pedas dan asam menyegarkan TANPA santan. Kuah merahnya yang encer namun kaya rempah (daun kunyit, serai) sangat nendang dan membuat keringat bercucuran, cocok untuk makan siang yang membangkitkan selera.",
        image: "riau_asam_pedas_ikan_baung.jpeg",
        prepTime: "20 Menit",
        cookTime: "30 Menit",
        servings: 4,
        difficulty: "Mudah",
        ingredients: [
            "1 kg Ikan Baung / Patin / Gabus",
            "1 Liter Air",
            "## Bumbu Halus (Merah)",
            "100 gr Cabai Merah Giling (Haluskan 20 cabai)",
            "8 Bawang Merah, 4 Bawang Putih",
            "2 cm Jahe, 2 cm Kunyit",
            "1 sdt Terasi Bakar",
            "## Aromatik & Asam",
            "3 keping Asam Kandis / Air Asam Jawa",
            "1 lembar Daun Kunyit",
            "1 batang Serai",
            "1 buah Tomat Merah"
        ],
        instructions: [
            "**Tumis Bumbu**: Tumis bumbu halus dengan sedikit minyak sampai matang dan berubah warna merah tua (tanak). Masukkan daun kunyit dan serai.",
            "**Rebus Kuah**: Tuang air. Masukkan asam kandis. Didihkan.",
            "**Masak Ikan**: Masukkan ikan baung. Masak dengan api sedang. Biarkan kuah meresap ke daging ikan.",
            "**Finishing**: Masak sampai kuah sedikit menyusut dan merah berminyak (minyak cabai keluar). Masukkan tomat. Angkat.",
            "**Rasa**: Harus seimbang antara Pedas Nendang dan Asam Segar."
        ],
        tips: [
            "**Warna Merah**: Kunci Asam Pedas Riau adalah warna kuah yang MERAH MENYALA. Gunakan cabai merah keriting banyak.",
            "**Tanpa Santan**: Masakan ini sangat ringan tapi kaya rasa rempah.",
            "**Asam Kandis**: Asam kandis (kulit buah kering) memberikan rasa asam yang pekat tanpa membuat kuah 'butek' seperti asam jawa.",
            "**Terasi**: Sedikit terasi bakar memperkuat rasa umami kuah pedas.",
            "**Ikan Baung**: Teksturnya mirip lele tapi lebih besar dan dagingnya sangat lembut manis."
        ]
    },
    {
        slug: "mie-sagu-selatpanjang",
        province: "Riau",
        title: "Mie Sagu Goreng (Selatpanjang)",
        description: "Mie kenyal yang terbuat dari pati sagu, makanan pokok khas Selatpanjang, Riau. Dimasak goreng dengan tauge, kucai, dan ikan teri atau udang, bertekstur kenyal unik (chewy).",
        image: "riau_mie_sagu_goreng.jpeg",
        prepTime: "15 Menit",
        cookTime: "15 Menit",
        servings: 2,
        difficulty: "Mudah",
        ingredients: [
            "300 gr Mie Sagu Kering (Rendam air dingin, tiriskan)",
            "100 gr Tauge",
            "1 ikat Daun Kucai (Potong panjang)",
            "50 gr Ikan Teri (Goreng kering) atau Udang",
            "2 butir Telur (Orak arik)",
            "## Bumbu Halus",
            "3 Bawang Merah, 2 Bawang Putih",
            "5 Cabai Merah (Haluskan kasar)",
            "1 sdm Kecap Ikan / Saus Tiram"
        ],
        instructions: [
            "**Siapkan Mie**: Cuci mie sagu, rendam sebentar sampai lemas, tiriskan. Jangan direbus lama nanti hancur jadi lem.",
            "**Tumis**: Tumis bumbu halus sampai wangi. Masukkan udang/teri dan telur orak arik.",
            "**Masak Mie**: Masukkan mie sagu dan tauge. Beri sedikit air (percikan saja) agar lembab. Aduk rata.",
            "**Bumbui**: Tambahkan kecap asin/ikan, garam, lada. Masak sampai mie bening kenyal. Terakhir masukkan KUCAI. Aduk sebentar. Angkat.",
            "**Sajikan**: Taburi bawang goreng dan teri garing."
        ],
        tips: [
            "**Tekstur**: Mie Sagu punya tekstur sangat kenyal (chewy) dan transparan, berbeda dengan mie tepung terigu. Bebas Gluten.",
            "**Kucai**: Kucai (Chives) adalah sayuran wajib untuk Mie Sagu, memberikan aroma bawang ringan.",
            "**Rendam Air**: Mie sagu kering cukup direndam air dingin (bukan air panas) agar tidak lengket.",
            "**Teri**: Taburan teri goreng garing memberikan kontras tekstur dengan mie yang kenyal.",
            "**Selatpanjang**: Kota penghasil sagu terbesar di Riau, mie ini adalah sarapan pokok di sana."
        ]
    },

    // --- KEPULAUAN RIAU (3 Recipes) ---
    {
        slug: "mie-lendir-kepri-premium",
        province: "Kepulauan Riau",
        title: "Mie Lendir (Kuah Kacang Ubi)",
        description: "Mie kuning dengan kuah kacang kental yang gurih manis. Tekstur 'lendir' yang legendaris berasal dari Ubi Jalar rebus yang dihaluskan, bukan bahan pengental buatan. Menu sarapan favorit di Tanjungpinang.",
        image: "kepulauan_riau_mie_lendir.jpeg",
        prepTime: "30 Menit",
        cookTime: "30 Menit",
        servings: 4,
        difficulty: "Sedang",
        ingredients: [
            "500 gr Mie Kuning Basah (Celur air panas)",
            "Tauge (Seduh)",
            "Telur Rebus (Iris)",
            "## Bahan Kuah Kental",
            "250 gr Ubi Jalar (Kukus & Haluskan) - Kunci Tekstur",
            "100 gr Kacang Tanah (Goreng & Haluskan)",
            "800 ml Air",
            "50 gr Gula Merah",
            "## Bumbu Halus Kuah",
            "5 Bawang Merah, 3 Bawang Putih",
            "10 Cabai Merah, 1 ruas Kencur",
            "1 batang Serai (Geprek), 2 Daun Salam"
        ],
        instructions: [
            "**Rebus Kuah**: Didihkan air. Masukkan bumbu halus, serai, daun salam. Masak sampai wangi.",
            "**Kentalkan**: Masukkan UBI JALAR halus dan KACANG TANAH halus. Aduk rata menggunakan whisk agar tidak menggumpal.",
            "**Simmer**: Masak api kecil sampai kuah mengental licin dan meletup-letup. Masukkan Gula Merah dan Garam. Rasa harus dominan Manis-Gurih kacang.",
            "**Penyajian**: Tata mie dan tauge. Siram kuah kental panas. Toping telur rebus dan irisan cabai rawit hijau (untuk pedas)."
        ],
        tips: [
            "**Ubi Jalar**: Pilih ubi jalar warna oranye atau kuning untuk warna kuah yang cantik alami.",
            "**Kencur**: Sedikit kencur memberikan aroma segar yang menyeimbangkan rasa manis ubi.",
            "**Kekentalan**: Kuah Mie Lendir harus benar-benar kental (seperti saus), tidak encer.",
            "**Cabe Rawit Iris**: Karena kuahnya manis, irisan cabai rawit hijau (cabe potong) wajib ada untuk yang suka pedas.",
            "**Blender Halus**: Pastikan ubi dan kacang diblender sangat halus agar tekstur kuah mulut 'smooth'."
        ]
    },
    {
        slug: "gonggong-rebus-kepri",
        province: "Kepulauan Riau",
        title: "Gonggong Rebus Original (Siput Laut)",
        description: "Siput laut khas Kepulauan Riau yang direbus sederhana dengan jahe dan garam, dinikmati dengan cara mencungkil dagingnya dari cangkang dan dicocol sambal khas. Teksturnya kenyal mirip cumi.",
        image: "kepulauan_riau_gonggong_rebus_original.jpeg",
        prepTime: "20 Menit",
        cookTime: "15 Menit",
        servings: 4,
        difficulty: "Mudah",
        ingredients: [
            "1 kg Siput Gonggong Segar (Cuci sikat kulitnya)",
            "Air untuk merebus",
            "2 ruas Jahe (Geprek)",
            "1 sdm Garam",
            "## Sambal Cocolan Khas",
            "20 Cabai Rawit Merah",
            "3 siung Bawang Putih (Mentah)",
            "1 sdm Cuka / Air Jeruk Nipis",
            "Gula, Garam, sedikit air hangat"
        ],
        instructions: [
            "**Bersihkan**: Cuci gonggong berkali-kali sampai air bening dan tidak berpasir.",
            "**Rebus**: Didihkan air di panci. Masukkan Jahe dan Garam. Masukkan Gonggong.",
            "**Waktu Masak**: Rebus selama 15-20 menit. Jangan terlalu lama agar daging tidak alot. Saat matang, kaki siput (duri kecil) akan mudah ditarik.",
            "**Buat Sambal**: Blender kasar cabai dan bawang putih. Campur dengan gula, garam, dan cuka. Rasanya harus Pedas-Asam menyengat.",
            "**Cara Makan**: Gunakan tusuk gigi. Tusuk daging gonggong, tarik keluar perlahan. Buang bagian kotoran hitam di ujung ekor (opsional). Cocol sambal."
        ],
        tips: [
            "**Endemik**: Gonggong (Laevistrombus canarium) hanya mudah ditemukan di perairan Kepri dan Bangka.",
            "**Nutrisi**: Tinggi protein dan dipercaya sebagai aphrodisiac lokal.",
            "**Jangan Overcook**: Kunci kenikmatan adalah perebusan pas. Jika terlalu lama, daging susut dan keras.",
            "**Kotoran**: Ada bagian hitam di ujung ekor (pencernaan) yang beberapa orang buang karena berpasir, tapi ada juga yang memakannya.",
            "**Sambal Cuka**: Sambal khas gonggong wajib pakai cuka putih, aroma asamnya memotong amis siput."
        ]
    },
    {
        slug: "laksa-kuah-lemak-kepri",
        province: "Kepulauan Riau",
        title: "Laksa Kuah Lemak (Laksa Melayu)",
        description: "Laksa khas Melayu Kepulauan Riau yang menggunakan mie sagu putih tebal. Disajikan dalam kuah santan gurih yang dicampur dengan daging ikan tongkol halus sehingga tekstur kuahnya kental, 'berpasir', dan sangat kaya rasa (lemak). Wajib dimakan dengan sambal belacan pedas.",
        image: "kepulauan_riau_laksa_kuah_lemak.jpeg",
        prepTime: "40 Menit",
        cookTime: "40 Menit",
        servings: 4,
        difficulty: "Sedang",
        ingredients: [
            "500 gr Mie Laksa / Mie Sagu Putih Tebal",
            "## Bahan Kuah Ikan",
            "500 gr Ikan Tongkol / Tenggiri (Kukus, ambil dagingnya, haluskan)",
            "1 Liter Santan",
            "## Bumbu Halus (Sangrai Dulu)",
            "1 sdm Ketumbar, 1/2 sdt Jinten, 1/2 sdt Lada",
            "8 Bawang Merah, 4 Bawang Putih",
            "3 cm Kunyit, 2 cm Jahe, 2 cm Lengkuas",
            "3 butir Kemiri",
            "1 sdt Terasi Bakar (Belacan)",
            "## Rempah Daun",
            "2 batang Serai, 3 Daun Jeruk, 1 Daun Kunyit"
        ],
        instructions: [
            "**Sangrai Rempah**: Sangrai ketumbar dan jinten sebelum dihaluskan agar wangi.",
            "**Tumis Bumbu**: Tumis bumbu halus dan rempah daun sampai matang harum.",
            "**Masak Kuah**: Masukkan daging ikan halus ke dalam tumisan. Aduk rata. Tuang santan.",
            "**Simmer**: Masak api sedang sambil diaduk. Kuah laksa Melayu biasanya agak kental (pekat) karena serat daging ikan yang menyatu dengan santan. Bumbui garam gula.",
            "**Penyajian**: Taruh mie di mangkuk. Siram kuah panas pekat. Sajikan dengan mentimun cincang dan Sambal Belacan."
        ],
        tips: [
            "**Kuah Ikan**: Ciri Laksa Kepri adalah kuahnya yang 'berpasir' dari daging ikan tumbuk, bukan kuah santan licin.",
            "**Mie Sagu**: Gunakan mie sagu yang kenyal, bukan mie kuning biasa.",
            "**Sambal Belacan**: Laksa ini wajib pedas dengan sambal terasi (belacan) khas Melayu.",
            "**Bungkus**: Tradisi laksa Kepri sering dijual dibungkus daun kelapa/daun pisang membentuk kerucut.",
            "**Ikan Tongkol**: Gunakan ikan tongkol/tenggiri segar untuk rasa kuah yang manis umami."
        ]
    }
];
